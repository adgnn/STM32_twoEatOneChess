...
switch(gameState){
    case 1:
        if(Player1->playerState == Move){
            //移动player1的某个子(清空cx，cy坐标的棋子，画gocx，gocy坐标的棋子)
            //更新CB[5][4]
            //更新Player1对应的chessX、chessY（Player1 -> chessX[whichChess] = gocx）
            CB[]
        }
    case 2:
...
void choosechess1(int number, enum gameState state, struct Player* Player1){
	switch(state){
			case 1: //玩家1的回合
					if(Player1->playerState == chooseChess){
							if(Player1->chessState[number] == 1){
									// 若玩家1的第1个棋子还没被吃
									whichChess = number+1;//更新选定棋子编号
									cx = Player1 -> chessX[number];
									cy = Player1 -> chessY[number]; //更新全局变量cx、cy为玩家1的第1个棋子的坐标
									Player1 -> playerState = chooseDir; //将玩家1的状态更新为待选方向
							}
					}
					else{
...
void choosechess2(int number, enum gameState state, struct Player* Player2){
	switch(state){
			case 2: //玩家2的回合
					if(Player2->playerState == chooseChess){
							if(Player2->chessState[number] == 1){
									// 若玩家2的第1个棋子还没被吃
									whichChess = number+1;//更新选定棋子编号
									cx = Player2 -> chessX[number];
									cy = Player2 -> chessY[number]; //更新全局变量cx、cy为玩家1的第1个棋子的坐标
...
void choosedir1(struct direction DIR[4], enum gameState state, struct Player* Player1){
	int tempX,tempY;
	switch(state){
			case 1:
					if(Player1->playerState == chooseDir){
							if(Player1->chessState[whichChess] == 1){
									...
									if(cx >= 0){
											...
											if(CB[tempX][tempY] == 0){
													//若对应位置没有棋子，可以移动
													gocx = tempX + DIR[UP/DOWN/LEFT/RIGHT].dx;
													gocy = tempY + DIR[UP/DOWN/LEFT/RIGHT].dy;
													//选定了落子的坐标，玩家状态进入Move
													Player1->playerState = Move;
											}
											else{
...
void choosedir2(struct direction DIR[4], enum gameState state, struct Player* Player2){
	int tempX,tempY;
	switch(state){
			case 2:
					if(Player2->playerState == chooseDir){
